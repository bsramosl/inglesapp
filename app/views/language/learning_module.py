    ...
    if request.method == 'POST':
        ...
        if action == 'create':
            try:
                ...
                if form.is_valid():
                    LearningModule.objects.create(**form.cleaned_data)
        ...
        if action == 'del':
            try:
                ...
                module = LearningModule.objects.get(pk=id)
                module.status = False
                module.save()
        ...
        if 'action' in request.GET:
            ...
            data['page_subtitle'] = u"Learning Module Management"
            data['title_table'] = u"Learning Module List"
            data['modules'] = LearningModule.objects.filter(status=True).order_by('order')
            return render(request, "language/learning_module_view.html", data)