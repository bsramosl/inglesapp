...
from app.forms import LearningModuleForm
    ...
    if request.method == 'POST':
        ...
        if action == 'create':
            try:
                ...
                if form.is_valid():
                    LearningContent.objects.create(**form.cleaned_data)
        ...
        if action == 'del':
            try:
                ...
                module = LearningContent.objects.get(pk=id)
                module.status = False
                module.save()
        ...
        if 'action' in request.GET:
            ...
            if action == 'edit':
                try:
                    data['title'] = u'Learning Content'
                    data['id'] = id = request.GET.get('id',None)
                    data['action'] = 'edit'
                    module = LearningContent.objects.get(pk=id)
                    data['form'] =  LearningModuleForm(initial=model_to_dict(module))
            ...
            if action == 'create':
                data['title'] = u'Learning Content'
                data['form'] = LearningModuleForm()
                data['action'] = 'create'
                template = get_template("layout/form.html")
                json_content = template.render(data)
                return JsonResponse({"result":True, "data":json_content})
        else:
            data['tittle'] = u"Learning Module"
            data['page_subtitle'] = u"Learning Content Management"
            data['title_table'] = u"Learning Content List"
            data['modules'] = LearningContent.objects.filter(status=True).order_by('order')
            return render(request, "language/learning_content_view.html", data)